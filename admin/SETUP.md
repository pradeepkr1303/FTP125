...
## Pre Nov-20th
  ...
  * Create  repository for the 10 teams
    ...
    * `git clone --bare ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/FTP125`
    * 'cd FTP125`
    ...
    * For each team, nn = 01 to 10
      ...
      * For windows
        ...
        * cd ftpnn
        * use VS Code, find/replace in files to do the following: replacing FTP125 -> ftpnn, FTP125 -> FTPnn
        * Rename directory restservice/leavemanager/src/main/java/com/hexaware/FTP125/ to restservice/leavemanager/src/main/java/com/hexaware/ftpnn/
      * For Mac OS; replace nn with the team-id in the following three commands; use xxnn.sh
        * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/FTPnn.git`
        * `cd ftpnn`      
        * `find . -type f -not -path '*/\.*' -not -path "*/node_modules/*" -exec sed -i '' -e 's/FTP125/ftp[nn]/g' {} \; -exec sed -i '' -e 's/FTP125/FTP[nn]/g' {} \;`
        * `mv restservice/leavemanager/src/main/java/com/hexaware/FTP125/ restservice/leavemanager/src/main/java/com/hexaware/ftp[nn]/`
        * `mv restservice/leavemanager/src/test/java/com/hexaware/FTP125/ restservice/leavemanager/src/test/java/com/hexaware/ftp[nn]/`
        * `mv restservice/integration-test/src/test/java/com/hexaware/FTP125 restservice/integration-test/src/test/java/com/hexaware/ftp[nn]`
  ...
  * Databases - 3 per team
    * Integration and Staging instances
      ...
      * 
    * `CREATE DATABASE FTP125;` -- XX from 01 to 10
    * `CREATE USER 'FTP125'@'%' IDENTIFIED BY 'FTP125';`
    * `GRANT ALL ON FTP125.* TO 'FTP125'@'%';`
    * `select Host, User, Password from mysql.user order by user;`
    * `select * from mysql.db order by Db;`
  * Jenkins jobs - 3 per team
    * FTP125-10-UNIT
    * FTP125-30-INTEGRATION
    * FTP125-50-STAGING
    * For each team, nn = 01 to 10
      * Create a tab FTPnn
...
Create a "FTP125" as project board and add users through add members link at Trello board
